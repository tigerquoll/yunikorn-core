...
// updateAskMaxPriority rescan (throughput-phase1.md, item 1).
func assertMaxPriorityConsistent(t *testing.T, app *Application) {
	...
	for _, req := range app.sortedRequests {
...
// longer holds. removeAsksInternal("") wipes sa.requests, sortedRequests and the pending histogram
...
func TestRollbackAllocationAskNotTracked(t *testing.T) {
	...
	app.RLock()
	assert.Equal(t, len(app.sortedRequests), 0, "rollback of an untracked ask must not insert into sortedRequests")
