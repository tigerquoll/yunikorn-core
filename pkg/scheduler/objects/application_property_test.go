...
func runPropertyFuzz(t *testing.T, seed int64) int { //nolint:funlen
	...
	// would not fire, but under -tags askinvariants (O(#requests) rescan on every Unlock) or a
	// loaded/CI machine it could - and if it does, the wipe is invisible to the reference model,
	...
	for i := 0; i < steps; i++ {
		...
		case 7: // RollbackAllocation - revert a confirmed allocation back to a pending ask
			...
			// (application.go ~line 875), which is the function that does incPendingPriority plus
			...
			// and remains rollback-eligible. deallocateAsk must not resurrect such an ask into
			// sortedRequests, where tryAllocate would then try to schedule something the application
			// does not track any more, so the reference model below only marks the key pending again
			// when it is still tracked.
			...
			if key, ok := pickRandomFilteredKey(rng, confirmedKeys, func(k string) bool {
				...
				if _, rollbackErr := app.RollbackAllocation(key); rollbackErr == nil {
					...
					if _, tracked := keyPriority[key]; tracked {
						...
						// leave the pending histogram and sortedRequests completely untouched.
						ghostRollbacks++
					}
				}
			}
	...
	// bookkeeping (e.g. decPendingPriority) has states to be caught in.
