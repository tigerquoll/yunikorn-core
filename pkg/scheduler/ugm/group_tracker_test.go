...
func TestGTIncreaseTrackedResource(t *testing.T) {
	...
	groupTracker := newGroupTracker(user.User, newUGMEvents(eventSystem), nil)
	...
	groupTracker.increaseTrackedResource(path1, TestApp1, usage1, user.User, nil)
	...
	groupTracker.increaseTrackedResource(path2, TestApp2, usage2, user.User, nil)
	...
	groupTracker.increaseTrackedResource(path3, TestApp3, usage3, user.User, nil)

	usage4, err := resources.NewResourceFromConf(map[string]string{"mem": "20M", "vcore": "20"})
	if err != nil {
		t.Errorf("new resource create returned error or wrong resource: error %t, res %v", err, usage3)
	}
	groupTracker.increaseTrackedResource(path4, TestApp4, usage4, user.User, nil)
	...
	groupTracker = nil
	groupTracker.increaseTrackedResource(path1, TestApp1, usage1, user.User, nil) // no panic
}
...
func TestDecreaseAllTrackedResourceUsage(t *testing.T) {
	...
	eventSystem := mock.NewEventSystem()
	groupTracker := newGroupTracker(testUser.User, newUGMEvents(eventSystem), nil)

	usage1, err := resources.NewResourceFromConf(map[string]string{"mem": "10M", "vcore": "10"})
	assert.NilError(t, err)
	groupTracker.increaseTrackedResource(path1, TestApp1, usage1, testUser.User, nil)
	groupTracker.increaseTrackedResource(path1, TestApp2, usage1, testUser.User, nil)
...
func TestGTSetAndClearMaxLimits(t *testing.T) {
	...
	eventSystem.Reset()
	groupTracker.setLimits(path1, resources.Multiply(usage1, 5), 5, nil)
	groupTracker.setLimits(path5, resources.Multiply(usage1, 10), 10, nil)
	...
	groupTracker.increaseTrackedResource(path1, TestApp2, usage1, user.User, nil)
	groupTracker.increaseTrackedResource(path1, TestApp3, usage1, user.User, nil)
	...
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy1, nil), path1expectedHeadroom))
	...
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy5, nil), path5expectedHeadroom))
	assert.Assert(t, groupTracker.canRunApp(hierarchy1, TestApp4, nil))

	// lower limits
	groupTracker.setLimits(path1, usage1, 1, nil)
	groupTracker.setLimits(path5, resources.Multiply(usage1, 2), 1, nil)
	lowerChildHeadroom := resources.NewResourceFromMap(map[string]resources.Quantity{
		"mem":   -20000000,
		"vcore": -20000,
	})
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy1, nil), lowerChildHeadroom))
	lowerParentHeadroom := resources.NewResourceFromMap(map[string]resources.Quantity{
		"mem":   -10000000,
		"vcore": -10000,
	})
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy5, nil), lowerParentHeadroom))
	assert.Assert(t, !groupTracker.canRunApp(hierarchy1, TestApp4, nil))
	assert.Assert(t, !groupTracker.canRunApp(hierarchy5, TestApp4, nil))

	// clear limits
	eventSystem.Reset()
	groupTracker.clearLimits(path1, nil)
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy1, nil), lowerParentHeadroom))
	assert.Assert(t, resources.Equals(groupTracker.headroom(hierarchy5, nil), lowerParentHeadroom))
	assert.Assert(t, !groupTracker.canRunApp(hierarchy1, TestApp4, nil))
	assert.Assert(t, !groupTracker.canRunApp(hierarchy5, TestApp4, nil))
	groupTracker.clearLimits(path5, nil)
	assert.Assert(t, groupTracker.headroom(hierarchy1, nil) == nil)
	assert.Assert(t, groupTracker.headroom(hierarchy5, nil) == nil)
	assert.Assert(t, groupTracker.canRunApp(hierarchy1, TestApp4, nil))
	assert.Assert(t, groupTracker.canRunApp(hierarchy5, TestApp4, nil))
	assert.Equal(t, 2, len(eventSystem.Events))
	assert.Equal(t, si.EventRecord_REMOVE, eventSystem.Events[0].EventChangeType)
...
func TestGTCanRunApp(t *testing.T) {
	...
	user := security.UserGroup{User: "test", Groups: []string{"test"}}
	groupTracker := newGroupTracker(user.User, newUGMEvents(mock.NewEventSystemDisabled()), nil)
	...
	hierarchy1 := strings.Split(path1, configs.DOT)
	assert.Assert(t, groupTracker.canRunApp(hierarchy1, TestApp1, nil))
