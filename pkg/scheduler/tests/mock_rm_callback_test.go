...
// RM via UpdateAllocation. It is either a "new allocation" entry (ApplicationID/AllocationKey/NodeID
// set, TerminationType empty) or a "release" entry (ApplicationID/AllocationKey/TerminationType set,
// NodeID empty). See golden_trace_test.go for how this is used to build a golden decision-trace.
//
// Both kinds carry the application ID because an allocation key only identifies the work, not whose
// work it is: a regression that releases the right key against the wrong application - which is
...
type mockRMCallback struct {
	...
	// trace is an ordered, append-only record of every allocation/release decision observed via
