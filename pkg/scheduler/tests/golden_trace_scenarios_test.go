...
// leafSortConfig builds a single-leaf partition whose leaf uses the given application.sort.policy.
...
// nodeSortConfig builds a single-leaf partition with the given nodesortpolicy type.
...
func runGang(t *testing.T, ms *mockScheduler, seq *createTimeSeq, probe *traceProbe) {
	...
	waitForAllocationGone(t, ms, scNode, "g-placeholder")
}
...
func runPreemption(t *testing.T, ms *mockScheduler, seq *createTimeSeq, probe *traceProbe) {
	...
	victimKey := preemptionVictim(probe, ms.mockRM.getTrace())
	...
	assert.NilError(t, err)
	waitForAllocationGone(t, ms, scNode, victimKey)
	ms.scheduler.MultiStepSchedule(2)
