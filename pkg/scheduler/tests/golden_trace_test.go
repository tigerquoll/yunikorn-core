...
// (applicationID, allocationKey, nodeID) for every new allocation, and (applicationID,
// allocationKey, terminationType) for every release. That sequence is stored in
// testdata/golden_trace.json and compared verbatim.
//
...
// UPDATE_GOLDEN they report what the run actually produced and carry on instead of failing, and so do
// the workload invariants that describe the old behaviour (traceProbe.pin). What does NOT give way is
// the workload's own preconditions - an ask that never registers, an allocation that never leaves a
// node - because those mean the script itself no longer does what it says, and the fix for that is
// to update the workload rather than to bless a new golden.
//
...
//
// That last point matters because an application's requests are ordered by priority descending then
// creation time ascending (Allocation.LessThan); asks that tie on both fall back to insertion order
// into the sorted list, which is an implementation detail rather than a defined ordering. The
// workload avoids ties so that the expected order follows from the documented rule alone.
//
...
// waitForAllocationGone blocks until the node no longer holds the allocation. Used after telling the
// core that the RM has released an allocation (a placeholder replacement or a preemption victim):
// that release is processed on the allocation-event goroutine as well, so the capacity it frees is
// not visible to the caller until the node has actually dropped it.
func waitForAllocationGone(t *testing.T, ms *mockScheduler, nodeID, allocationKey string) {
	t.Helper()
	err := common.WaitForCondition(10*time.Millisecond, time.Duration(goldenTimeout)*time.Millisecond, func() bool {
		node := ms.getNode(nodeID)
		return node != nil && node.GetAllocation(allocationKey) == nil
	})
...
type traceProbe struct {
	t     *testing.T
	ms    *mockScheduler
	queue string // leaf queue the barrier applications of sync() are parked in
...
//     it.
//
// The barrier application is inert: it holds no asks, so it has no pending resources, and
// filterOnPendingResources drops it before any queue sorting or allocation attempt sees it. It never
// appears in the trace, which records allocations and releases only.
...
// pin asserts a property of the workload that the golden's meaning rests on - the headroom a phase
// leaves behind, an ask that must still be waiting. Like the trace lengths these describe the
// behaviour being pinned, so they report and carry on when regenerating rather than blocking the
...
func TestGoldenDecisionTrace(t *testing.T) { //nolint:funlen
	...
	pt := newTraceProbe(t, ms, queueMain)
	...
	waitForAllocationGone(t, ms, nodeMain, "g-placeholder")
	...
	pt.pin(len(liveOnNodeMain) == liveAtRestart,
	...
	// rather than against the comment above it.
	free := ms.getNode(nodeMain).GetAvailableResource()
	pt.pin(free.Resources[siCommon.Memory] == nodeMainCapMem-recoveredMem && free.Resources[siCommon.CPU] == nodeMainCapVcore-recoveredVcore,
	...
	// any timer or timeout trigger. The requirement being pinned is that the abandoned asks are
	// discarded rather than left behind for a later cycle to allocate or trip over.
	//
	...
	// node cannot take more work, and it is asserted rather than asserted-by-comment.
	// -----------------------------------------------------------------------------------------
	free = ms.getNode(nodeMain).GetAvailableResource()
	pt.pin(free.Resources[siCommon.Memory] == 2*failAskMem && free.Resources[siCommon.CPU] == 2*failAskVcore,
	...
	victimKey := preemptionVictim(pt, ms.mockRM.getTrace())
	...
	assert.NilError(t, err)
	waitForAllocationGone(t, ms, nodePreempt, victimKey)
	ms.scheduler.MultiStepSchedule(2)
...
// this needs to establish is that the entry is a release of one of the two candidates, because the
// key is fed straight back into the release that confirms the preemption. An empty trace is fatal
// even when regenerating: there is no key to feed back, so the workload cannot continue.
func preemptionVictim(p *traceProbe, trace []traceEntry) string {
	p.t.Helper()
	if len(trace) == 0 {
		p.t.Fatal("no decision to read a preemption victim from")
	}
	victim := trace[len(trace)-1]
	p.pin(victim.TerminationType != "" && (victim.AllocationKey == "low-1" || victim.AllocationKey == "low-2"),
