...
# Check the lock annotations. The list covers every package of the scheduler now that all locked
# structs are annotated, it is kept as a variable because a new package with locks in it has to be
# annotated before it can be checked: an unannotated package still triggers lock balance errors
# which would fail the check. Note that the "+checklocks:" requirements of a function are only
# enforced for callers in the listed packages.
